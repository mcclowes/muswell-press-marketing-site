...
const blockWidths = objMap(blockHeights, (key, val) => mixins.px(mixins.num(val) * 0.45));

const Wrapper = styled.div`
	${ mixins.bpEither(
		"height",
		objMap(vars.dim.nav.height, (key, val) =>
			mixins.px(mixins.num(val) - 2 * mixins.num(vars.dim.nav.margin[key])),
		),
	) }
	position: relative;
	display: flex;
...
const Block = styled.div`
	${ mixins.bpEither("height", blockHeights) } ${ mixins.bpEither("width", blockWidths) };
`;
